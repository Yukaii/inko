  ...
  return (
    <div className="text-text-primary">
      <div className="mx-auto flex max-w-6xl flex-col gap-8">
        <header className="grid gap-4 overflow-hidden rounded-[28px] border border-[var(--border-subtle)] bg-[radial-gradient(circle_at_top_left,rgba(255,107,53,0.18),transparent_32%),linear-gradient(135deg,var(--bg-card),var(--bg-page))] p-8 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
        ...
        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          ...
          {decks.map((deck) => (
            ...
            >
              ...
              <div>
                <h2 className="m-0 text-2xl font-bold [font-family:var(--font-display)] text-text-primary group-hover:text-accent-orange transition-colors">{deck.title}</h2>
              ...
              <div className="mt-auto flex items-center justify-between text-xs text-text-secondary">
                ...
                <span className="font-bold text-accent-orange">{t("community.library.view_deck")}</span>
              </div>
            </Link>
          ))}
        </section>
