  ...
  return (
    <div className="wordbank-container">
      ...
      <header className="wordbank-header">
        ...
        <p className="page-subtitle">Manage your decks and vocabulary</p>
      </header>
        ...
        >
          {decks.map((deck, index) => (
            <button
              ...
              className={`deck-tile${selectedDeckId === deck.id ? " selected" : ""}`}
            ...
            >
              ...
              <div className="deck-tile-actions">
                <Link to={`/practice/${deck.id}`} onClick={(e) => e.stopPropagation()} className="tile-action-link">
                  <span>Practice</span>
                  <kbd className="tile-shortcut">p</kbd>
                </Link>
                <button
                  type="button"
                ...
                >
                  <span>Edit</span>
                  <kbd className="tile-shortcut">Enter</kbd>
                </button>
              </div>
            </button>
          ))}
          ...
          <button
            ...
            className="deck-tile-new"
            onClick={() => setShowNewDeckModal(true)}
            tabIndex={focusedDeckIndex === decks.length ? 0 : -1}
          >
            <span className="plus-icon">+</span>
            <span className="new-deck-text">New Deck</span>
          </button>
        </div>
      </section>
      ...
      {selectedDeckId && (
        <section className="add-words-section">
          <div className="section-header">
            <h2 className="section-title">Add to {activeDeck?.name ?? "Deck"}</h2>
          </div>

          {/* Tabs */}
          <div className="zen-tabs" role="tablist" aria-label="Add words tabs">
            <button
              type="button"
              role="tab"
          ...
          {addTab === "single" ? (
            ...
            <div className="import-area">
              ...
              <button type="button" className="submit-btn" onClick={handleBulkImport} disabled={!importText.trim()}>
                Import Words
              </button>
            </div>
          )}
        ...
        <section className="words-section">
          ...
          {words.length > 0 ? (
              ...
              {words.map((word) => (
                <div key={word.id} className="word-card">
                  <div className="word-main">
                    <span className="word-target" lang="ja">{word.target}</span>
                  ...
                  <div className="word-meta">
                    <div className="word-tags">
                      {(word.tags ?? []).map((t: string) => (
                        <span key={t} className="word-tag">{t}</span>
                      ))}
                    </div>
                    <button
                      type="button"
            ...
            <div className="empty-state-card">
              <p>No words yet.</p>
              <p className="empty-state-hint">Add words using the form above.</p>
            </div>
          )}
        </section>
      )}
